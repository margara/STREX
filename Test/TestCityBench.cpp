...
void cityBenchQueryOnly() {
	const char type[] = "seq";
	const char kb[] = "/Volumes/RamDisk/rdf/static.ttl";
...
void cityBenchQuery(int numQuery, int numEvents, int timeBetweenEvents) {
	...
	const char type[] = "seq";
	std::string kbString = std::string("/Volumes/RamDisk/Q" + std::to_string(numQuery) + "/rdf/static.ttl");
	...
	// FIXME: this was the code used in the evaluation to mimic the "and" semantics with two rules
//	RDFTRexRuleParser parser1;
//	RDFTRexRuleParser parser2;
//	RulePkt *r = new RulePkt(true);
//	r->setCompositeEventTemplate(new CompositeEventTemplate(2000));
//	r->addRootPredicate(1000, NULL, 0);
//	trex->processRulePkt(r);
//	std::ifstream regola1("./citybench_q1.txt");
//	std::ifstream regola2("./citybench_q1_2.txt");
//	std::stringstream buffer1;
//	std::stringstream buffer2;
//	buffer1 << regola1.rdbuf();
//	buffer2 << regola2.rdbuf();
//	std::string str1 = buffer1.str();
//	std::string str2 = buffer2.str();
//	parser1.parse(str1, store, trex, constructor);
//	parser2.parse(str2, store, trex, constructor);
	...
	for (int i=0; i<numEvents; i++) {
		stringstream strstr;
		strstr << "/Volumes/RamDisk/Q" << numQuery << "/rdf/event_" << i << ".ttl";
	...
	double avgTime = std::chrono::duration_cast<std::chrono::nanoseconds>(end-begin).count() / (1000000.0 * numEvents);
